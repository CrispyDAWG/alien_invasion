...
class Explosions(Sprite):
    alien_explosion_images = [pg.image.load(f"images_other/exp{n}.png") for n in range(2)]
...
class Ship_explosion(Sprite):
    ship_explosion_images = [pg.image.load(f"images_other/ship_exp{n}.png") for n in range(2)]
    ...
    def __init__(self, ai_game):
        ...
        self.timer = Timer(images=Ship_explosion.ship_explosion[Ship_explosion.n], delta=1000, start_index=Ship_explosion.n, loop_continuously=False)
        self.image = self.timer.current_image()
        self.rect = self.image.get_rect()
